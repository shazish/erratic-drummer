...
public class SoundMaker implements Runnable {
	...
	int [][] drumMapSequenceGroup; // [beat #, inst_group #]
	int [][] drumMapSequenceInst; // [beat #, instrument #]
	...
	//float [] vol = {0.6f, 0.7f, 0.8f, 0.9f, 1.0f};
	float [] vol = {0.3f, 0.45f, 0.6f, 0.75f, 0.9f};
	float finalVolume;
	
	int tempo, tempoAfterSettingsImplemented;
	int totalPatternRepeats;
	int patternLength, patternLengthAfterSettingsImplemented;
	...
	SoundMaker (Main _context) {
		...
		instGroupPriority = new int [4];
	 	initializeSoundPool(0);	 		 	
	}
	...
	public void playSequence(){
		...
		while (currentPatternRepeats <= totalPatternRepeats)	{								
			...
			if (randomLength)
				patternLength = patternLengthAfterSettingsImplemented;
			for (int j = 0 ; j < patternLength ; j++ ){
				if (!playKeyPressed | fwdKeyPressed)
					break outerloop;
					try {
						for (int i = 0 ; i < MAX_SIMULTANEOUS_INST ; i++) {								
							if (variableVolume) {
								finalVolume = vol[rand.nextInt(5)];
								//Log.d("SoundMaker", "finalVolume: " + finalVolume);
							}
							else {
								finalVolume = 0.9f;
							}
							if (drumMapSequenceGroup[j][i]!= -1 & drumMapSequenceInst[j][i]!= -1 ) {								
									// float vol = rand.nextFloat() / 2; // possible lag for weaker CPUs?
									soundPool.play(loadedSoundPool
											[ drumMapSequenceGroup[j][i] ][ drumMapSequenceInst[j][i] ]
											, finalVolume, finalVolume, 1, 0, 1);																	
							}									
					}
						if (randomTempo) 
							tempo = tempoAfterSettingsImplemented;
						Thread.sleep(15000/tempo); 
					} catch (Exception e) { 
						Log.d("SoundMaker", e.toString());
					}
			}
			if (lockKeyPressed) { // Checks LockKey status at the beginning and end of each loop
				currentPatternRepeats = totalPatternRepeats - 1;
	...
	public void generateSequence(){
		...
		if (randomLength) {
			patternLengthAfterSettingsImplemented = 
				MIN_RIFF_LENGTH + rand.nextInt(MAX_RIFF_LENGTH - MIN_RIFF_LENGTH);
		}
		...
		for (int i = 0 ; i < MAX_RIFF_LENGTH ; i++ ){		
			// Clean up previous composition
			for (int m = 0 ; m < MAX_SIMULTANEOUS_INST ; m++) {
					drumMapSequenceGroup[i][m] = -1; 
					drumMapSequenceInst[i][m] = -1; 
			}
			...
			for (int m = 0 ; m < MAX_SIMULTANEOUS_INST ; m++) { 
				...
				// chooses one of the instruments based on values of instGroupPriority.
				...
				for (int x = 0; x < 4; x++) {
					...
					if ( temp_priority_randomizer < temp_priority_sum_keeper ) {
						drumMapSequenceGroup[i][m] = x;
						break;
					}					
				}
				// chooses one of the instruments based on the length of the arrow that correlates to the chosen instrument 
				if ( loadedSoundPool [drumMapSequenceGroup[i][m]].length != 0 )
					drumMapSequenceInst[i][m] = rand.nextInt( loadedSoundPool [drumMapSequenceGroup[i][m]].length ); 

			}
			// Log.d("Instrument chosen", drumMapSequenceGroup[i][0] + " " + drumMapSequenceInst[i][0] + " -- "
			//		+ drumMapSequenceGroup[i][1] + " " + drumMapSequenceInst[i][1] );			
		}
	...
	public void run() {
		...
		while(running)
			if (playKeyPressed) {
				...
				if (justInitialized == false) playSequence();
	...
	public void initializeSoundPool(int bankID){		
		soundPool = new SoundPool (20, AudioManager.STREAM_MUSIC, 0); // MAX_SIMULTANEOUS_INST * 2 is enough
		// soundPoolMute = new SoundPool (1, AudioManager.STREAM_MUSIC, 0); // MAX_SIMULTANEOUS_INST * 2 is enough	
		initializeSoundPoolVars();
		try {
			...
			if (bankID == 1) {
				...
				**/
				...
				soundPool.play(loadedMuteSound, 0, 0, 0, -1, 1); // starts playing mute sound for sync purposes
	...
	public void disposeSoundpool(){
    	...
    	drumMapSequenceGroup = null;
    	drumMapSequenceInst = null;
